...
namespace Example.Events.ServiceBus
{
    public class ServiceBusTopicEventListener: IEventListener
    {
        ...
        private readonly ManagementClient mClient;

        public ServiceBusTopicEventListener(ServiceBusClient client, ILogger<ServiceBusTopicEventListener> logger, IEnumerable<IEventHandler> eventHandlers, ManagementClient mClient)
        {
            this.client = client;
        ...
        public async Task StartAsync(string topicName, string subscriptionName, CancellationToken cancellationToken)
        {
            try
            {
                var result = await mClient.GetSubscriptionAsync(topicName, subscriptionName);
            }
            catch (Microsoft.Azure.ServiceBus.MessagingEntityNotFoundException e)
            {
                await mClient.CreateSubscriptionAsync(topicName, subscriptionName);
            }
